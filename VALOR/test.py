...
def validate(model, val_dataloaders, opts, global_step, total_step):
    ...
    for task, loader in val_dataloaders.items():
        ...
        val_log = validate_single(model, loader, task.split('--')[0], opts, global_step, total_step,
                                  task.split('--')[1])
...
def validate_qa(model, eval_loader, task_str, opts, global_step, dset_name):
    ...
    if 'tv' in task:
        ...
        if dist.get_rank() == 0:
            json.dump(submit_files_t_v,
                      open(os.path.join(output_dir, f'step{global_step}_tv_pred_submited_{dset_name}.json'), 'w'))
    ...
    if dist.get_rank() == 0:
        ...
        if opts.contra_type == 'fine':
            if 'tv' in task:
                ...
                score_matrix_t_v = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_v, maskA, maskB, weightA,
                                                                                weightB)
            ...
            if 'tva' in task:
                if opts.late_fusion:
                    ...
                    score_matrix_t_va = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_v, maskt, maskv,
                                                                                     weightt, weightv) + \
                                        get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_a, maskt, maska,
                                                                                     weightt, weighta)
                    ...
                    if opts.fineweight_type == 'none':
                        ...
                        weightA = get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2)
                        weightB = torch.cat((get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2),
                                             get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)),
                                            dim=1)

                    score_matrix_t_va = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_va, maskA, maskB,
                                                                                     weightA, weightB)
            ...
            if 'ta' in task:
                ...
                score_matrix_t_a = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_a, maskA, maskB, weightA,
                                                                                weightB)
            ...
            if 'va' in task:
                ...
                weightB = get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)
                score_matrix_va = get_model_attr(model, 'compute_fine_matrix')(feat_v, feat_a, maskA, maskB, weightA,
                                                                               weightB)
            ...
            if 'vta' in task:
                ...
                weightA = get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2)
                weightB = torch.cat((get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2),
                                     get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)), dim=1)
                score_matrix_vta = get_model_attr(model, 'compute_fine_matrix')(feat_v, feat_ta, maskA, maskB, weightA,
                                                                                weightB)
            ...
            if 'atv' in task:
                ...
                weightB = torch.cat((get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2),
                                     get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2)), dim=1)
                score_matrix_atv = get_model_attr(model, 'compute_fine_matrix')(feat_a, feat_tv, maskA, maskB, weightA,
                                                                                weightB)
            ...
            if 'tva' in task:
                if opts.late_fusion:
                    score_matrix_t_va = torch.matmul(feat_t, feat_v.permute(1, 0)) + torch.matmul(feat_t,
                                                                                                  feat_a.permute(1, 0))
                else:
                    feat_va = F.normalize(get_model_attr(model, 'va_fusion')(torch.cat((feat_v, feat_a), dim=-1)),
                                          dim=-1)
        ...
        if dist.get_rank() == 0:
            if opts.contra_type == 'fine':
                if 'tv' in contra_task:
                    ...
                    weightA = get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2)
                    weightB = get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2)
                    score_matrix_t_v = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_v, maskA, maskB,
                                                                                    weightA, weightB)
                ...
                if 'tva' in contra_task:
                    if opts.late_fusion:
                        ...
                        if opts.fineweight_type == 'none':
                            ...
                            weightA = get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2)
                            weightB = torch.cat((
                                                get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2),
                                                get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(
                                                    2)), dim=1)
                        score_matrix_t_va = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_va, maskA, maskB,
                                                                                         weightA, weightB)
                ...
                if 'ta' in contra_task:
                    ...
                    weightA = get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2)
                    weightB = get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)
                    score_matrix_t_a = get_model_attr(model, 'compute_fine_matrix')(feat_t, feat_a, maskA, maskB,
                                                                                    weightA, weightB)
                ...
                if 'va' in task:
                    ...
                    weightA = get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2)
                    weightB = get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)
                    score_matrix_va = get_model_attr(model, 'compute_fine_matrix')(feat_v, feat_a, maskA, maskB,
                                                                                   weightA, weightB)
                ...
                if 'vta' in task:
                    ...
                    weightB = torch.cat((get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2),
                                         get_model_attr(model, 'fine_weight_mapper')['audio'](feat_a).squeeze(2)),
                                        dim=1)
                    score_matrix_vta = get_model_attr(model, 'compute_fine_matrix')(feat_v, feat_ta, maskA, maskB,
                                                                                    weightA, weightB)
                ...
                if 'atv' in task:
                    ...
                    weightB = torch.cat((get_model_attr(model, 'fine_weight_mapper')['text'](feat_t).squeeze(2),
                                         get_model_attr(model, 'fine_weight_mapper')['video'](feat_v).squeeze(2)),
                                        dim=1)
                    score_matrix_atv = get_model_attr(model, 'compute_fine_matrix')(feat_a, feat_tv, maskA, maskB,
                                                                                    weightA, weightB)
