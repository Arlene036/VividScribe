...
def load_from_pretrained_dir(opts):
    ...
    if 'video_frame_embedding' in checkpoint:
        checkpoint['video_frame_embedding'][:, pretrain_cfg.video_sample_num:] = checkpoint['video_frame_embedding'][:,
                                                                                 pretrain_cfg.video_sample_num - 1].clone()
    if 'audio_frame_embedding' in checkpoint:
        checkpoint['audio_frame_embedding'][:, pretrain_cfg.audio_sample_num:] = checkpoint['audio_frame_embedding'][:,
                                                                                 pretrain_cfg.audio_sample_num - 1].clone()
    ...
    if opts.video_resolution != pretrain_cfg['video_resolution']:
        if opts.video_encoder_type.startswith('clip'):
            ...
            src_oth = F.interpolate(src_oth.reshape(grid_size, grid_size, vision_width).permute(2, 0, 1).unsqueeze(0),
...
def conduct_train(model, optimizer, train_loader, val_loaders, LOGGER, TB_LOGGER, opts, start_step=0,
                  verbose_time=False):
    if dist.get_rank() == 0:
    ...
    for step, (name, batch) in enumerate(train_loader):
        ...
        if (opts.dataset_mix_type == 'accum' and (step + 1) % ndata == 0) or opts.dataset_mix_type in ['round-robin',
                                                                                                       'random']:
            global_step += 1
...
def create_train_dataloaders(opts):
    ...
    for name, (loader, ratio) in dataloaders_dict.items():
        ...
        LOGGER.info(
            f" loader {name} , ratio {ratio} , bs_pergpu {loader.batch_size}, n_workers {loader.num_workers}, epoch {epoch}")
