...
class VALOR(VALORModel):
    ...
    def __init__(self, opts):
        ...
        if self.txt_encoder_type.startswith('clip') and self.video_encoder_type.startswith(
                'clip') and opts.init_clip_head:
    ...
    def compute_fine_matrix(self, featA, featB, maskA, maskB, weightA, weightB):
        if featB.shape[0] > 1200:
            ...
            for i in range(slices):
                output.append(self.compute_fine_matrix_slice(featA[i * freq:i * freq + freq], featB,
                                                             maskA[i * freq:i * freq + freq], maskB,
                                                             weightA[i * freq:i * freq + freq], weightB))
    ...
    def forward_pt(self, batch, task, compute_loss=True):
        ...
        if contra_task != []:
            ...
            if compute_loss:
                ...
                if self.contra_type == 'fine':
                    ...
                    if 'tva' in contra_task:
                        ...
                        if self.late_fusion:
                            ...
                            score_matrix_tva = self.compute_fine_matrix(feat_t, feat_v, maskt, maskv, weightt,
                                                                        weightv) + \
                            ...
                            if self.fineweight_type == 'none':
                                ...
                                weightB = torch.cat(torch.ones_like(feat_v[:, :, 0]), torch.ones_like(feat_v[:, :, 0]),
                                                    dim=1)
                            else:
                                weightA = self.fine_weight_mapper['text'](feat_t).squeeze(2)
                                weightB = torch.cat((self.fine_weight_mapper['video'](feat_v).squeeze(2),
                                                     self.fine_weight_mapper['audio'](feat_a).squeeze(2)), dim=1)
                    ...
                    if 'vta' in contra_task:
                        maskA = torch.ones_like(feat_v[:, :, 0])
                        maskB = torch.cat(((txt_tokens_contra != 0).long().cuda(), torch.ones_like(feat_a[:, :, 0])),
                                          dim=1)
                        feat_ta = torch.cat((feat_t, feat_a), dim=1)
                        weightA = self.fine_weight_mapper['video'](feat_v).squeeze(2)
                        weightB = torch.cat((self.fine_weight_mapper['text'](feat_t).squeeze(2),
                                             self.fine_weight_mapper['audio'](feat_a).squeeze(2)), dim=1)
                    ...
                    if 'atv' in contra_task:
                        ...
                        maskB = torch.cat(((txt_tokens_contra != 0).long().cuda(), torch.ones_like(feat_v[:, :, 0])),
                                          dim=1)
                        feat_tv = torch.cat((feat_t, feat_v), dim=1)
                        weightA = self.fine_weight_mapper['audio'](feat_a).squeeze(2)
                        weightB = torch.cat((self.fine_weight_mapper['text'](feat_t).squeeze(2),
                                             self.fine_weight_mapper['video'](feat_v).squeeze(2)), dim=1)
                    ...
                    if 'tva' in contra_task:
                        ...
                        if self.late_fusion:
                            score_matrix_tva = torch.matmul(feat_t, feat_v.permute(1, 0)) + torch.matmul(feat_t,
                                                                                                         feat_a.permute(
                                                                                                             1, 0))
                        else:
                ...
                lo = []
                for i in (
                contra_loss_tva, contra_loss_tv, contra_loss_ta, contra_loss_va, contra_loss_vta, contra_loss_atv):
                    if i is not None:
                        lo.append(i)
        ...
        if caption_task != []:
            ...
            if 'tva' in caption_task:
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output_tva = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, audio_input,
                                                                     casual=True)
            ...
            if 'tv' in caption_task:
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output_tv = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, None,
                                                                    casual=True)
            ...
            if 'ta' in caption_task:
                ...
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output_ta = self.forward_multimodal_encoder(txt_input, task_prompt, None, audio_input,
                                                                    casual=True)
        ...
        if mlm_task != []:
            ...
            if 'tva' in mlm_task:
                ...
                mlm_output_tva = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, audio_input,
                                                                 casual=False)
            ...
            if self.contra_type == 'fine':
                ...
                if 'tva' in task:
                    if self.late_fusion:
                        ...
                        if self.fineweight_type == 'none':
                            weightA = torch.ones_like(feat_t[:, :, 0])
                            weightB = torch.cat((torch.ones_like(feat_v[:, :, 0]), torch.ones_like(feat_a[:, :, 0])),
                                                dim=1)
                        else:
                            weightA = self.fine_weight_mapper['text'](feat_t).squeeze(2)
                            weightB = torch.cat((self.fine_weight_mapper['video'](feat_v).squeeze(2),
    ...
    def forward_cap_scst(self, batch, task):
        ...
        if 'tv' in task:
            ...
            caption_loss_tv = self.reward_loss(evaluation_dict_sample['generated_sequences_t_v'], logprobs_t_v_sample,
                                               rewards)
        ...
        if 'tva' in task:
            ...
            rewards = reward_sample - reward_greedy * self.get_alpha()
            rewards = torch.from_numpy(rewards).float().cuda()
            caption_loss_tva = self.reward_loss(evaluation_dict_sample['generated_sequences_t_va'],
                                                logprobs_t_va_sample, rewards)
        ...
        if 'ta' in task:
            ...
            rewards = reward_sample - reward_greedy * self.get_alpha()
            rewards = torch.from_numpy(rewards).float().cuda()
            caption_loss_ta = self.reward_loss(evaluation_dict_sample['generated_sequences_t_a'], logprobs_t_a_sample,
                                               rewards)
    ...
    def forward_cap_single(self, batch, task, cache=None, compute_loss=True, cache_first=False):
        ...
        if compute_loss:
            ...
            if 'tva' in task:
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, audio_input,
                                                                 casual=True, full_masker=self.full_masker)
                ...
                if self.label_smoothing > 0:
                    caption_loss_tva = self.label_smoothing_loss(prediction_scores_caption,
                                                                 txt_labels_caption[txt_labels_caption != -1])
                else:
                    caption_loss_tva = F.cross_entropy(prediction_scores_caption,
                                                       txt_labels_caption[txt_labels_caption != -1])

            if 'tv' in task:
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, None, casual=True,
                                                                 full_masker=self.full_masker)
                caption_output_txt = caption_output[:, :txt_tokens.shape[1], :]
                masked_output = caption_output_txt[txt_labels_caption != -1]
                prediction_scores_caption = self.cls(masked_output)
                if self.label_smoothing > 0:
                    caption_loss_tv = self.label_smoothing_loss(prediction_scores_caption,
                                                                txt_labels_caption[txt_labels_caption != -1])
                else:
                    caption_loss_tv = F.cross_entropy(prediction_scores_caption,
                                                      txt_labels_caption[txt_labels_caption != -1])
            ...
            if 'ta' in task:
                ...
                task_prompt = self.get_task_prompt('describe the video with natural language',
                                                   batch_size) if self.use_task_prompt else None
                caption_output = self.forward_multimodal_encoder(txt_input, task_prompt, None, audio_input, casual=True,
                                                                 full_masker=self.full_masker)
                caption_output_txt = caption_output[:, :txt_tokens.shape[1], :]
                masked_output = caption_output_txt[txt_labels_caption != -1]
                prediction_scores_caption = self.cls(masked_output)
                if self.label_smoothing > 0:
                    caption_loss_ta = self.label_smoothing_loss(prediction_scores_caption,
                                                                txt_labels_caption[txt_labels_caption != -1])
                else:
                    caption_loss_ta = F.cross_entropy(prediction_scores_caption,
                                                      txt_labels_caption[txt_labels_caption != -1])
            ...
            if not self.multimodal_use_cross_attn:
                ...
                caption_output, cache = self.forward_multimodal_encoder(txt_input, prompt_input, \
                                                                        ...
                                                                        cache=cache, use_cache=True, casual=True,
                ...
                caption_output = self.forward_multimodal_encoder(txt_input, prompt_input, \
                                                                 video_input, audio_input, \
                                                                 cache=cache, use_cache=False, casual=True,
                                                                 cache_first=cache_first,
                                                                 cache_type=self.caption_type)
    ...
    def generate_cap(self, batch, task, mode='none'):
        ...
        if 'tv' in task:
            ...
            batch['prompt_input'] = self.get_task_prompt('describe the video with natural language',
                                                         batch_size) if self.use_task_prompt else None
            if self.beam_size > 1:
    ...
    def _adjust_tensor(self, batch_size, beam_size, tensor, selected_beam):
        ...
        if tensor.dim() == 4:  # b,n,c
            ...
            tensor = torch.gather(tensor.view(batch_size, beam_size, h, n, c), 1, \
                                  selected_beam.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(batch_size, beam_size,
                                                                                                 h, n, c))
    ...
    def forward_qa_single(self, batch, task, compute_loss=True):
        ...
        if compute_loss:
            ...
            if 'tva' in task:
                ...
                if self.use_task_prompt:
                    task_prompt = self.get_task_prompt('describe the video with natural language', batch_size)
                ...
                qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, audio_input,
                                                            casual=True, full_masker=self.full_masker)
                ...
                loss = F.cross_entropy(prediction_scores_qa, txt_labels_qa, ignore_index=-1, reduction='none').reshape(
                    b, n)
                txt_labels_qa = txt_labels_qa.reshape(b, n)
            ...
            if 'tv' in task:
                if self.use_task_prompt:
                    ...
                    task_prompt = question_tokens
                qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, None, casual=True,
                                                            full_masker=self.full_masker)
                qa_output_txt = qa_output[:, :txt_tokens.shape[1], :]
            ...
            if 'ta' in task:
                if self.use_task_prompt:
                    ...
                    task_prompt = question_tokens
                qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, None, audio_input, casual=True,
                                                            full_masker=self.full_masker)
                qa_output_txt = qa_output[:, :txt_tokens.shape[1], :]
    ...
    # def forward_qa_single(self, batch, task, cache=None, compute_loss=True, cache_first=False):
    #     batch = defaultdict(lambda: None, batch)
    #     txt_tokens = batch['txt_tokens']
    #     video_pixels = batch['video_pixels']
    #     audio_spectrograms = batch['audio_spectrograms']
    ...
    #
    #         if 'tva' in task:
    #             task_prompt = self.get_task_prompt('answer the question using visual and audio cues',
    #                                                batch_size) if self.use_task_prompt else question_tokens
    #             qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, audio_input,
    #                                                         casual=True, full_masker=self.full_masker)
    #             qa_output_txt = qa_output[:, :txt_input.shape[1], :]
    #             masked_output = qa_output_txt[txt_labels_qa != -1]
    #             prediction_scores_qa = self.cls(masked_output)
    #
    #             if self.label_smoothing > 0:
    #                 qa_loss_tva = self.label_smoothing_loss(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #             else:
    #                 qa_loss_tva = F.cross_entropy(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #
    #         if 'tv' in task:
    #             task_prompt = self.get_task_prompt('answer the question using visual cues',
    #                                                batch_size) if self.use_task_prompt else question_tokens
    #             qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, video_input, None, casual=True,
    #                                                         full_masker=self.full_masker)
    #             qa_output_txt = qa_output[:, :txt_tokens.shape[1], :]
    ...
    #             prediction_scores_qa = self.cls(masked_output)
    #             if self.label_smoothing > 0:
    #                 qa_loss_tv = self.label_smoothing_loss(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #             else:
    #                 qa_loss_tv = F.cross_entropy(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #
    #         if 'ta' in task:
    #             task_prompt = self.get_task_prompt('answer the question using audio cues',
    #                                                batch_size) if self.use_task_prompt else question_tokens
    #             qa_output = self.forward_multimodal_encoder(txt_input, task_prompt, None, audio_input, casual=True,
    #                                                         full_masker=self.full_masker)
    #             qa_output_txt = qa_output[:, :txt_tokens.shape[1], :]
    ...
    #             prediction_scores_qa = self.cls(masked_output)
    #             if self.label_smoothing > 0:
    #                 qa_loss_ta = self.label_smoothing_loss(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #             else:
    #                 qa_loss_ta = F.cross_entropy(prediction_scores_qa, txt_labels_qa[txt_labels_qa != -1])
    #
    #         lo = []
    #         for i in (qa_loss_tva, qa_loss_tv, qa_loss_ta):
    #             if i is not None:
    ...
    #
    #         if not self.multimodal_use_cross_attn:
    #             qa_output, cache = self.forward_multimodal_encoder(txt_input, prompt_input, video_input, audio_input,
    #                                                                cache=cache, use_cache=True, casual=True,
    #                                                                cache_first=cache_first,
    #                                                                cache_type=self.caption_type)
    #         else:
    #             qa_output = self.forward_multimodal_encoder(txt_input, prompt_input, video_input, audio_input,
    #                                                         cache=cache, use_cache=False, casual=True,
    #                                                         cache_first=cache_first, cache_type=self.caption_type)
    #
    #         qa_output_txt = qa_output[:, :txt_input.shape[1], :]
    #         qa_output_txt = qa_output_txt[:, -1]
    #         prediction_scores_qa = self.cls(qa_output_txt)
    #         return prediction_scores_qa, cache
